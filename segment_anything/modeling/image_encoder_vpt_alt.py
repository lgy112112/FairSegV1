...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        if self.num_vpt_tokens > 0:
            ...
            self.visual_prompt_embeddings = nn.Parameter(torch.zeros(1, self.num_vpt_tokens * 2, embed_dim))
            nn.init.uniform_(self.visual_prompt_embeddings.data, -val, val)
            # # deep visual prompts
            self.deep_visual_prompt_embeddings = nn.Parameter(torch.zeros(len(self.blocks) - 1, self.num_vpt_tokens * 2, embed_dim))
        ...
        for idx, blk in enumerate(self.blocks):
            if idx == 0:
                ...
                if idx <= self.deep_visual_prompt_embeddings.shape[0]:
                    ...
                    x = torch.cat([deep_prompt_emb, x[:, 2 * self.num_vpt_tokens:]], dim=1)  # (B, num_tokens + H * W, embed_dim)
                x = blk(x)

        # split visual prompts and image tokens
        visual_prompts = x[:, :self.num_vpt_tokens, :]  # (B, N, C)
        x = x[:, 2 * self.num_vpt_tokens:, :].view(B, H, W, C)
...
class PromptedBlock(nn.Module):
    ...
    def prompted_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        visual_prompts = x[:, :2 * self.num_tokens, :]  # (B, num_tokens, embed_dim)
        x = x[:, 2 * self.num_tokens:, :]  # (B, H * W, embed_dim)
        L = L - 2 * self.num_tokens
        # print(L, H * W)
        ...
        if self.window_size > 0:
            ...
            visual_prompts = visual_prompts.reshape((-1, 2 * self.num_tokens, C))
        ...
        if self.window_size > 0:
            ...
            visual_prompts = visual_prompts.reshape(-1, B, 2 * self.num_tokens, C)
